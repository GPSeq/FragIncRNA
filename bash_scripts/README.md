...
# K-mer Comparison QC Script

## `summarize_kmer_comparison_qc.sh`

Summarizes the already-combined k-mer comparison TSV files across multiple
k-mer sizes. These input files are the per-k outputs generated by
`summarize_kmer_matches_by_transcript.sh`, with transcript metadata followed by
per-genome matched k-mer counts and matched k-mer ratios.
...
- Input directory: `../../kmers_comparison`
- Input files: all `.tsv` files in the input directory
- Expected columns:
  - `transcript_name`
  - `gene_name`
  - `gene_id`
...
- `../../kmers_comparison/qc/kmer_transcript_pass_counts_by_k.tsv`
- `../../kmers_comparison/qc/kmer_shared_summary_by_k.tsv`
- `../../kmers_comparison/qc/kmer_shared_gene_summary_by_k.tsv`
- `../../kmers_comparison/qc/shared_kmer_transcripts/kmer_shared_transcripts_strict_by_k.tsv`
- `../../kmers_comparison/qc/shared_kmer_transcripts/kmer_shared_transcripts_basic_by_k.tsv`
- `../../kmers_comparison/qc/status_matrices/kmer_transcript_status_matrix_k<K>.tsv`

QC thresholds:

- Strict default: matched k-mer ratio `>= 0.95`
- Basic default: matched k-mer ratio `>= 0.80`

Thresholds can be changed with:
...
- `PASS_BASIC_KMER`: matched k-mer ratio is at least the basic threshold but
  below the strict threshold.
- `LOW_KMER`: matched k-mer ratio is below the basic threshold.

Output meaning:

- `kmer_transcript_pass_counts_by_k.tsv` reports, for every transcript and
  k-mer size, how many genomes pass the strict and basic k-mer thresholds.
- `kmer_shared_summary_by_k.tsv` summarizes total transcripts, total unique
  gene names, and strict/basic shared transcript and gene-name counts per k.
- `kmer_shared_gene_summary_by_k.tsv` reports gene-level summaries per k using
  `gene_name`.
- Strict shared transcripts are transcripts passing the strict k-mer threshold
  in every genome.
- Basic shared transcripts are transcripts passing at least the basic k-mer
  threshold in every genome.

Notes:

- This script uses only matched k-mer counts and ratios from the combined TSV
  files. It does not calculate unique-hit or low-copy k-mer specificity, because
  those require the original `_kmers.tsv` files with the slash-separated
  `kmer_counts` column.
